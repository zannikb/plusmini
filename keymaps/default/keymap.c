...
state = update_tri_layer_state(state, _BASE, _FN);
return state;
} */

/* rotary encoder - add different functions for layers here */

/*#if defined(ENCODER_MAP_ENABLE)
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][1] = {
    [_BASE] =   { ENCODER_CCW_CW(_FN1, _BASE)  },
    [_FN1] =  { ENCODER_CCW_CW(_FN2, _FN1)  },
    [_FN2] =  { ENCODER_CCW_CW(_FN3, _FN2)  },
    [_FN3] =  { ENCODER_CCW_CW(_BASE, _FN3)  },
};
#endif */
...
/*bool encoder_update_user(uint8_t index, bool clockwise) {
  ...
  switch (index) {
    case 0:
      if (!clockwise && _BASE  < 10) {
        tap_code(_FN1);
      } else if (clockwise && _FN3  > 0){
        tap_code(_FN2);
      }
      //tap_code(DF(_BASE));
  } return true;
